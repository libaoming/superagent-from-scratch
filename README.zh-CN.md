...
**千行级、零框架、教学优先的现代 SuperAgent harness 复刻**——从 [bytedance/deer-flow](https://github.com/bytedance/deer-flow)（★76k）的心脏蒸馏而来，一个下午能读完。

[English README](README.md) · 拆解笔记见 [`notes/`](notes/)

> 🚧 **建设中**——S1 切片正在公开构建（loop 核心已完成，真实工具进行中）。每个切片交付时都带可离线跑的测试 + 一篇拆解笔记。
...
本仓库把 deer-flow 约 2,300 行的心脏重写成 **≤1,500 行零依赖 Python**（运行时依赖仅 `anthropic` + `pyyaml`），按五个切片线性展开，每片配离线测试 + 一篇讲 why 的拆解笔记。
...
| 切片 | 构建什么 | 学到什么 | 状态 |
|---|---|---|---|
| **S1** | agent 循环 + LLM 接缝 + 3 个真实工具 | agent 就是一个 while 循环。终止条件只有三种。为什么 `tool_result` 要以 `user` 角色回填。 | 🔨 进行中 |
...
| 上下文管理 | Summarization + DurableContext + TokenBudget + 输出限额 | Summarization + ToolOutputBudget 两件 |
| 防御 middleware | LoopDetection / ReadBeforeWrite / DanglingToolCall / Safety… | ToolErrorHandling 一件（每个砍掉的 = 一道练习） |
...
- **src/ ≤ 1,500 行**——超了说明在复刻产品功能而非教学核心，`wc -l` 可查。
