...
# 判定 prompt 抽成模块常量 = eval 闭环围着转的「单可变文件」教学版落点（SPEC #eval-loop M3）：
# 进化改这里，evals.py 的 TSV 用 label/commit hash 归因。
GOAL_JUDGE_PROMPT = "你是目标验收器：只回 YES 或 NO。判断给定对话是否已达成用户目标。"
...
def _goal_met(llm, state, goal: str) -> bool:
    ...
    # 整词判定（S5 审查 Y1 + S9 eval 基线 0.8 的两个假阳性）：startswith("YES") 会把
    # 「YESTERDAY …未达成」判 True；\b 保住「YES，/YES！」这类带标点的真阳性。
    return re.match(r"\s*YES\b", verdict, re.IGNORECASE) is not None
