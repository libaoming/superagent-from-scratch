...
|---|---|---|---|---|---|
| **1 system 指令** | skills 的 `system_block` + **S8 `<available-deferred-tools>` 纯名字清单**（两块都很薄——都是「目录常驻」） | `"你是目标验收器：只回 YES 或 NO…"` | `"你是对话压缩器：把历史压成一段摘要…"` | 继承调用方传入（教学版通常 `""`） | `"你是长期记忆更新器，只输出 JSON。"` |
